...
# App layout
# ------------------------------------------------------------------------------
#these are all static right now and should be changed to add user interactivety
app.layout = html.Div([
    html.Div([html.H1("Ocean Plastic Dashboard", className='title')]),
	 #first row
    html.Br(),
    html.Div([
        #first column
        html.Div([
	    html.H3("Filters",className='titleWOShadow'),
            html.H3("Filter by EC Challenge 2020 Dates:",className='filObjectTitle'),
            dcc.Slider(
		id="slct_year",
               	min = 2015,
               	max = 2018,
		...
		className='slider'
            ...
            html.H3("Filter by Debris Type:",className='filObjectTitle'),
            dcc.RadioItems(id = "debris_type_selector",
                options=[
                    {'label': 'All', 'value': 'All'}, #will have to change values to fix layout later
                    {'label': 'Hard Plastics', 'value': 'Hard Plastics'},#all the plastic value
                ...
                className = 'dcc_control radioFilObject'),
            dcc.Dropdown(
                id = "debris_type",
                options = debris_type_options,
                multi = True,
                placeholder = "Select Debris Type",
                className = 'dcc_control DropDownFil',
            ),
            html.H3("Filter By State:", className='filObjectTitle'),
            dcc.Dropdown(
                id = "slct_the_state",
                options = state_options,
                placeholder = "Select State",
                className = 'dcc_control filObject',
                value = 'CA'
            ),
            html.H3("Filter by State for Policy Enactment:", className='filObjectTitle'),
            dcc.Dropdown(
                id = "slct_state",
        ...
        #second column
        html.Div([
                html.H3("Map of Debris by Selected Dates",className = 'titleWOShadow'),
                dcc.Graph(
                id='bar_graph',
                figure={},
		className='bmap')
    ...
    #Second Row
    html.Div([
           html.H3("Percent of Debris in State by Year", className='titleWOShadow'),
	   html.Br(),
           dcc.Graph(
            id = 'debris_map',
            figure = {},
...
def update_text(slct_state):
    text = ['Please select a policy enactment to see information about it.']
